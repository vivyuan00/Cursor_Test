...
平台以AI舌诊引流、企微私域转化、医助服务、医生签字与支付履约为核心闭环。当前缺少统一的全链路转化漏斗分析能力，无法稳定支撑运营决策优化。
...
| G03 | 支持渠道产品级转化漏斗下钻分析（如舌诊产品） | 产品详细转化数据页上线，支持按日期查看各步骤转化率 |
...
| 数据分析师 | 渠道漏斗、医助服务转化、渠道产品转化分析 |
| 医助主管 | 查看本组医助服务人效与成交数据 |

## 3. 功能清单

| 编号 | 功能名称 | 功能描述 | 优先级 |
|------|----------|----------|--------|
| F01 | 漏斗日汇总模型 | 每日漏斗步骤F01~F10去重人数，按渠道/医助维度汇总 | P0 |
| F02 | 订单金额日汇总 | 按药物类型（汤剂/丸剂）、初复诊拆分的订单数与金额 | P0 |
| F03 | 医助成交日汇总 | 医助维度初诊/复诊订单数与金额汇总 | P0 |
| F04 | 渠道产品转化日汇总 | 渠道→子渠道→产品→版本层级核心指标与转化漏斗 | P0 |
| F05 | BI看板-每日数据统计 | 每日转化数据（F01~F10）、每日订单金额数据 | P0 |
...
| 数据条 | 数据截至：pt=yyyy-MM-dd（T+1为昨日业务日） | 与卡片副标题一致，禁止仅写「最新」 |
| 筛选条 | 业务日范围（默认最近7日/30日可切换）、渠道、医助、团队 | 渠道/医助选项受行级权限过滤 |
| 页脚 | 指标口径链接、反馈入口 | 反馈生成工单并关联看板/卡片ID |
...
> **注意**：数据表中AI工具转化的"加企微数"与医助转化的"加企微"为同一指标，数据应保持一致。
...
[6] > **统计维度**：以渠道（channel）→ 子渠道（sub_channel）→ 投放产品（product）→ 产品版本（product_version）为层级，在所选时间段内汇总核心指标。

| 指标 | 说明 |
|------|------|
| 首页PV | 该渠道/产品的落地页页面浏览量，比如舌诊首页、快测对话页 |
...
| 类别 | 要求 |
|------|------|
| 时效 | 默认T+1；每日任务在约定窗口内完成（建议06:00前） |
...
| 漏斗步骤 | F01~F10 | 从加企微到付费确认的完整转化链路各步骤 |
